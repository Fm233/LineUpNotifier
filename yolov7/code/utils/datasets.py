    ...
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        while True:
            ...
            try:
                ...
                if os.path.isfile(self.original_sources):
                    with open(self.original_sources, 'r') as f:
                        mut_sources = [x.strip() for x in f.read(
                        ).strip().splitlines() if len(x.strip())]
                else:
                    mut_sources = [self.original_sources]

                n = len(mut_sources)
                self.imgs = [None] * n
                # clean source names for later
                self.sources = [clean_str(x) for x in mut_sources]
                for i, s in enumerate(mut_sources):
                ...
                print(f'[Error] {e}')
            print(f"[Info] {detect_data.interval}秒后重试dataset的初始化！")
            time.sleep(detect_data.interval)
    ...
    def update(self, index, cap):
        ...
        while cap.isOpened():
            ...
            if n == 4:  # read every 4th frame
                ...
                self.avaliable = success
                if not success:
                    while True:
                        try:
                            print("[Warning] 获取图像失败！")
                            self.__init__(self.original_sources,
                                          self.img_size, self.stride)
                            return
                        except Exception as e:
                            print(f"[Error] {e}")

                        print(f"[Info] {detect_data.interval}秒后重试执行初始化！")
                        time.sleep(detect_data.interval)

                n = 0
            # time.sleep(1 if self.fps < 1 else 1 / self.fps)  # wait time
            time.sleep(0.02)
    ...
    def __next__(self):
        ...
        while True:
            ...
            try:
                ...
                detect_data.in_error = False
                return self.sources, img, img0, None
            except Exception as e:
                print(f'[Error] {e}')

            detect_data.in_error = True
            print(f'[Info] {detect_data.interval}秒后重试迭代！')
            time.sleep(detect_data.interval)
